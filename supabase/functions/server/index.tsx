  ...
  if (!supabase) {
    ...
    if (!supabaseKey) {
      throw new Error(
        ...
        'Key name: SERVICE_ROLE_KEY (or SUPABASE_SERVICE_ROLE_KEY). ' +
...
Deno.serve(async (req) => {
  ...
  const allowHeaders = accessControlRequestHeaders
    ...
    : 'Content-Type, Authorization, X-Requested-With, Accept, Origin, apikey, x-client-info, Prefer';

  const corsHeaders = {
    'Access-Control-Allow-Origin': requestOrigin && requestOrigin !== 'null' ? requestOrigin : '*',
  ...
  try {
    ...
    // Remove Supabase function prefixes so Hono sees only the internal route
    const prefixPatterns = ['/functions/v1', '/smart-action', '/hyper-worker'];
    for (const prefix of prefixPatterns) {
      if (pathname.startsWith(prefix)) {
        pathname = pathname.slice(prefix.length) || '/';
        console.log(`Normalized path (removed "${prefix}"): ${pathname}`);
      }
    }
